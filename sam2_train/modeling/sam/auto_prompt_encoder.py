...
class Prompt_Embedding_Generator(nn.Module):
    ...
    def forward(self,
        ...
        Arguments:
          img_embedding: torch.Tensor with shape (B, embed_dim, H, W)
          output_token: torch.Tensor with shape (B, N, embed_dim)

        Returns:
          torch.Tensor: new img embedding, with shape (B, embed_dim, H, W).
          torch.Tensor: object token, with shape Bx1x(embed_dim).
          torch.Tensor: new ouput token, with shape (B, N, embed_dim).
        """
        ...
        tokens = torch.cat([object_token, output_token], dim=1) # [b (1+N) d]
        ...
        #tokens[:, :1, :] 是 object token， tokens[:, 1:, :] 是 new_output_token
