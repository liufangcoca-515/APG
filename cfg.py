...
def parse_args():    
    ...
    # parser.add_argument('-video_length', type=int, default=4, help='video length')
    ...
    parser.add_argument('-memory_bank_size', type=int, default=1, help='sam 2d memory bank size')
    parser.add_argument('--apg', action='store_true', help='是否启用APG自动提示生成器')
    parser.add_argument('--train_stage', type=int, default=0, help='训练阶段：1=只训练APG，2=只训练主干，0=联合训练')
    parser.add_argument(
    '-data_path',
    type=str,
    default='./data/btcv',
    help='The path of segmentation data')
    parser.add_argument('--use_sam2_basepro', action='store_true', help='Use Sam2BasePro model from sam2_basepro_copy.py')
